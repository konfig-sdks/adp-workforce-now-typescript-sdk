...
import { readableStreamToString, AdpWorkforceNowError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new AdpWorkforceNowError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new AdpWorkforceNowError(e, e.response?.data)
                }
                if (innerError instanceof AdpWorkforceNowError) {
                    // Got "AdpWorkforceNowError" from the above try block
                    throw innerError;
                }
